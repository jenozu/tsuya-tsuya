...
<head>
  ...
  <style>
    ...
    .address-block, .address-block * { text-decoration: none !important; }
    .address-block a, a[x-apple-data-detectors] {
...
<body style="font-family: 'Tsuyanouchi Serif', Georgia, Cambria, 'Times New Roman', Times, serif; line-height: 1.7; color: #2D2A26; max-width: 600px; margin: 0 auto; padding: 0; background: #F2EFE9; font-variant-numeric: lining-nums tabular-nums;">
  ...
  <div style="background: #F9F8F4; padding: 40px 30px;">
    ...
    <div class="address-block" style="background: white; padding: 24px; margin-top: 20px; color: #4A4036; text-decoration: none;">
      <p style="margin: 0; color: #2D2A26; font-weight: 500; text-decoration: none;">${escapeHtml(addr.firstName)} ${escapeHtml(addr.lastName)}</p>
      <p style="margin: 8px 0 0; color: #4A4036; text-decoration: none;">${escapeHtml(addr.address)}</p>
      ${addr.addressLine2 ? `<p style="margin: 4px 0 0; color: #4A4036; text-decoration: none;">${escapeHtml(addr.addressLine2)}</p>` : ''}
      ${addr.unitNumber ? `<p style="margin: 4px 0 0; color: #4A4036; text-decoration: none;">Unit ${escapeHtml(addr.unitNumber)}</p>` : ''}
      <p style="margin: 4px 0 0; color: #4A4036; text-decoration: none;">${escapeHtml(addr.city)}, ${escapeHtml(addr.state)} ${escapeHtml(addr.postalCode)}</p>
      <p style="margin: 4px 0 0; color: #4A4036; text-decoration: none;">${escapeHtml(addr.country)}</p>
      ${addr.phone ? `<p style="margin: 8px 0 0; color: #4A4036; text-decoration: none;">Phone: ${escapeHtml(addr.phone)}</p>` : ''}
    </div>
  ...
  <div style="text-align: center; padding: 30px; color: #786B59; font-size: 11px; background: #E5E0D8;">
    <p style="margin: 0; text-transform: uppercase; letter-spacing: 0.15em;">TsuyaNoUchi</p>
